					...
					if(nodePiso1.hasAttributes())
						for(int z = 0; z < nodePiso1.getAttributes().getLength(); z++) {
							...
							case "Descripcion":
								...
								break;
							case "FormaPago":
								factura.setTipoPago(nodePiso1.getAttributes().item(z).getNodeValue());
							case "Fecha":
								factura.setFecha(nodePiso1.getAttributes().item(z).getNodeValue());
						...
						if(nodePiso2.hasAttributes())
							for(int y = 0; y < nodePiso2.getAttributes().getLength(); y++) {
								...
								case "Descripcion":
									...
									break;
								case "FormaPago":
									factura.setTipoPago(nodePiso1.getAttributes().item(y).getNodeValue());
								case "Fecha":
									factura.setFecha(nodePiso1.getAttributes().item(y).getNodeValue());
							...
							if(nodePiso3.hasAttributes())
								for(int a = 0; a < nodePiso3.getAttributes().getLength(); a++) {
									...
									case "Descripcion":
										...
										break;
									case "FormaPago":
										factura.setTipoPago(nodePiso1.getAttributes().item(a).getNodeValue());
									case "Fecha":
										factura.setFecha(nodePiso1.getAttributes().item(a).getNodeValue());
